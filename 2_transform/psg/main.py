...
def main() -> int:
    args = _parse_args()
    search_bases = _search_bases()
    pipeline_config = _load_pipeline_config(args.config, search_bases)

    input_files: list[Path]
    if args.input:
        input_files = [resolve_existing_path(args.input, search_bases)]
    else:
    ...
    if not _confirm_execution(plans, args.dry_run):
