...
def procesar_xpap_doc(texto_relevante: str, archivo: str):
    ...
    campos = [
        ...
        ("id", r"(?i)\b(?:Identificacion|Id)\b\s*[:|]?\s*\|?\s*(?:[A-Z]{1,4}\s*)?([0-9]{5,})\b(?=(?:\s*[A-Z]{1,4})?(?:\s+Adq\b)?\s*.*?\bPeso\b)"),
...
def procesar_xpap_rtf(texto_relevante: str, archivo: str):
    ...
    campos_comunes = [
        ...
        ("porcentaje_sueno_rem", r"(?is)DISTRIBUCION\s+DE\s+FASES\s+DE\s+SUENO:.*?REM\s*\|\s*(?:\d+\s*\|\s*)?[\d.,]+\s*\|\s*([\d.,]+)"),
        ("porcentaje_sueno_profundo", r"(?is)DISTRIBUCION\s+DE\s+FASES\s+DE\s+SUENO:.*?S3\s*\|\s*(?:\d+\s*\|\s*)?[\d.,]+\s*\|\s*([\d.,]+)"),
